...
Dir = '/glade/u/home/lgchen/ChesROMS/ChesROMS_UMD/data/forcing/era5/smallRegion_1hourly';

% Base date for ROMS forcing files: "days since 2000-01-01 00:00:00".
% LC: changed to "days since 1994-01-01 00:00:00"
%mybasedate = datenum(2000,1,1,0,0,0);
mybasedate = datenum(1994,1,1,0,0,0);
...
% Base date for ROMS forcing files: "days since 2000-01-01 00:00:00".
% LC: Base date for ROMS forcing files: "days since 1994-01-01 00:00:00".
%mybasedate = datenum(2000,1,1,0,0,0);
mybasedate = datenum(1994,1,1,0,0,0);
...
F( 1).Vname  = {'sustr', 'ewss'};
F( 1).Tname  = {'sms_time', 'valid_time'};
F( 1).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 1).output = 'ches_sms_era.nc';
F( 1).scale  = -1.0/(3*3600.0);


F( 2).Vname  = {'svstr', 'nsss'};
F( 2).Tname  = {'sms_time', 'valid_time'};
F( 2).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 2).output = 'ches_sms_era.nc';
F( 2).scale  = -1.0/(3*3600.0);

F( 3).Vname  = {'shflux', 'null'};
F( 3).Tname  = {'shf_time', 'valid_time'};
F( 3).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 3).output = 'ches_shflux_era.nc';
F( 3).scale  = -1.0/(3*3600.0);

F( 4).Vname  = {'swflux', 'null'};
F( 4).Tname  = {'swf_time', 'valid_time'};
F( 4).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 4).output = 'ches_swflux_era.nc';
F( 4).scale  = -100.0/(3*3600.0)*(24*3600.0);

F( 5).Vname  = {'swrad', 'ssr'};
F( 5).Tname  = {'srf_time',  'valid_time'};
F( 5).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 5).output = 'ches_swrad_era.nc';
F( 5).scale  = -1.0/(3*3600.0);

F( 6).Vname  = {'Uwind', 'u10'};
F( 6).Tname  = {'wind_time', 'valid_time'};
F( 6).input  = 'era5_inst_3h_199401.nc';
F( 6).output = 'ches_wind_era.nc';
F( 6).scale  = 1.0;

F( 7).Vname  = {'Vwind', 'v10'};
F( 7).Tname  = {'wind_time', 'valid_time'};
F( 7).input  = 'era5_inst_3h_199401.nc';
F( 7).output = 'ches_wind_era.nc';
F( 7).scale  = 1.0;

F( 8).Vname  = {'lwrad', 'str'};
F( 8).Tname  = {'lrf_time',  'valid_time'};
F( 8).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 8).output = 'ches_lwrad_era.nc';
F( 8).scale  = -1.0/(3*3600.0);

F( 9).Vname  = {'lwrad_down', 'strd'};
F( 9).Tname  = {'lrf_time',  'valid_time'};
F( 9).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F( 9).output = 'ches_lwrad_era.nc';
F( 9).scale  = -1.0/(3*3600.0);

F(10).Vname  = {'latent', 'slhf'};
F(10).Tname  = {'lhf_time',  'valid_time'};
F(10).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F(10).output = 'ches_latent_era.nc';
F(10).scale  = -1.0/(3*3600.0);

F(11).Vname  = {'sensible', 'sshf'};
F(11).Tname  = {'sen_time',  'valid_time'};
F(11).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F(11).output = 'ches_sensible_era.nc';
F(11).scale  = -1.0/(3*3600.0);

F(12).Vname  = {'cloud', 'tcc'};
F(12).Tname  = {'cloud_time',  'valid_time'};
F(12).input  = 'era5_inst_3h_199401.nc';
F(12).output = 'ches_cloud_era.nc';
F(12).scale  = 1.0;

F(13).Vname  = {'rain', 'tp'};
F(13).Tname  = {'rain_time',  'valid_time'};
F(13).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F(13).output = 'ches_rain_era.nc';
F(13).scale  = -1000.0/(3*3600.0);

F(14).Vname  = {'Pair', 'msl'};
F(14).Tname  = {'pair_time',  'valid_time'};
F(14).input  = 'era5_inst_3h_199401.nc';
F(14).output = 'ches_Pair_era.nc';
F(14).scale  = 0.01;

F(15).Vname  = {'Tair', 't2m'};
F(15).Tname  = {'tair_time',  'valid_time'};
F(15).input  = 'era5_inst_3h_199401.nc';
F(15).output = 'ches_Tair_era.nc';
F(15).scale  = 1.0;

F(16).Vname  = {'Qair', 'd2m'};         % Use temperature (t2m) and
F(16).Tname  = {'qair_time',  'valid_time'};  % dewpoint temperature (d2m)
F(16).input  = 'era5_inst_3h_199401.nc';       % to compute relative humidity
F(16).output = 'ches_Qair_era.nc';
F(16).scale  = 1.0;

F(17).Vname  = {'PAR', 'par'};
F(17).Tname  = {'srf_time',  'valid_time'};
F(17).input  = 'era5_accu_3h_increasingTo3h_199401.nc';
F(17).output = 'ches_PAR_era.nc';
...
%doFields = [1 2 3 4 5];
doFields = 1:17;
doFields = 1:16; % unable to find metadata for variable: PAR

% Various parameters.

spherical = true;                       % Spherical switch

LonMin    = -79;                     % GOM left-bottom longitude
LonMax    = -73;                      % GOM right-top   longitude
LatMin    = 34;                       % GOM left-bottom latitude
LatMax    = 41;                       % GOM right-top   latitude

FlipLat   = true;                       % Flip data in ERA NetCDF files
                                        % (see below information)

StrDay    = datenum('01-Jan-1994');     % starting day to process
EndDay    = datenum('30-Jan-1994');     % ending   day to process
...
% of the extracted grid (LonMin,LatMin)
 ...
 %                             'Gulf of Mexico Region'];
  S.Attributes(2).Value     = ['ECMWF ERA5 Dataset, ',           ...
                               'Chesapeake and Delaware Bay Region'];
...
%epoch = datenum('01-Jan-1970');  % "seconds since 1970-01-01" vs 'days since 1994-01-01'

% Set reference time for this application using the specified base date.

ref_time = (mybasedate - epoch);  % time difference or distance
disp(ref_time);
...
for n = doFields,
  ...
  while (year <= EndYear)  % for each current year
  % suffix  = strcat(num2str(year), '.nc');  % no need for this anymore;
...
% At 3 hour step don't change anything
      ...
      if (F(n).scale < 0)
        ...
        %\Ligang: added, as we use ERA5 data which are accumulated every 3 hour.
        % not like the original ERA-Interim, which are accumulated every 12
        % hour. 
        fieldfinal = field;  
...
% Write out data.
    ...
    end  % end of "for Rec=StrRec:EndRec,"
...
%  files.
  ...
  end  % end of "while (year <= EndYear)"

end  % end of "for n = doFields,"
