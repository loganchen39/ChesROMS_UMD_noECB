...
%SODA4_Dir = '/glade/u/home/lgchen/ChesROMS/ChesROMS_UMD/data/boundary/soda4.15.2-REGRIDED-ocean-levs38';
SODA3_Dir = '/glade/u/home/lgchen/ChesROMS/ChesROMS_UMD/data/boundary/soda3.4.2-REGRIDED-ocean';

GRDname = '/glade/u/home/lgchen/ChesROMS/ChesROMS_UMD/myscript/grid_UMD.nc';
BRYname = '/glade/u/home/lgchen/ChesROMS/ChesROMS_UMD/myscript/bry_ChesROMS-UMD_fromSODA3.nc';
...
%% set SODA4 files
%fl_soda4 = fullfile(SODA4_Dir, 'soda4.15.2_5dy_ocean_reg_1990_01_05.nc');
%fl_soda3 = dir(fullfile(SODA3_Dir, 'soda3.4.2_5dy_ocean_reg_????_??_??.nc'));
fl_soda3 = dir(fullfile(SODA3_Dir, 'soda3.4.2_5dy_ocean_reg*.nc'));
disp(fl_soda3)
nfiles = length(fl_soda3)
%disp(struct2table(fl_soda3))
%for k = 1:length(fl_soda3)
%    fprintf('%s\n', fullfile(fl_soda3(k).folder, fl_soda3(k).name));
%end
...
%  Set attributes for "bry_time".
  ...
  % avalue='seconds since 1990-01-01 00:00:00';
  avalue='days since 1980-01-01 00:00:00';  % units from SODA3
...
%  s=unix(['gunzip ',fileV]);
  ...
  fn_soda3 = fullfile(fl_soda3(n).folder, fl_soda3(n).name);
  disp(fn_soda3)
...
%  starts on 11-Oct-2006.
 ...
 %mydate=datestr(datenum('11-Oct-2006')+time/86400-0.5,0);
  time=nc_read(fn_soda3,'time');
  mydate=datestr(datenum('01-Jan-1980')+time,0);
...
%  Get Mercator grid.
  ...
  Tlon=nc_read(fn_soda3,'xt_ocean');
  Tlat=nc_read(fn_soda3,'yt_ocean');
  Tdepth=nc_read(fn_soda3,'st_ocean');

  Ulon=nc_read(fn_soda3,'xu_ocean');
  Ulat=nc_read(fn_soda3,'yu_ocean');
  Udepth=nc_read(fn_soda3,'st_ocean');

  Vlon=nc_read(fn_soda3,'xu_ocean');
  Vlat=nc_read(fn_soda3,'yu_ocean');
  Vdepth=nc_read(fn_soda3,'st_ocean');

% disp(size(Tlon));  % 720 1
% disp(size(Tlat));  % 330 1
% disp(size(Tdepth));  % 50 1

% disp(size(Ulon));  % 720 1
% disp(size(Ulat));  % 330 1
% disp(size(Udepth)); % 50 1
...
% Zeta=nc_read(fileT(1:lenT),'sossheig');
  Zeta=nc_read(fn_soda3,'ssh');
  Temp=nc_read(fn_soda3,'temp');
  Salt=nc_read(fn_soda3,'salt');
  Uvel=nc_read(fn_soda3,'u');
  Vvel=nc_read(fn_soda3,'v');
% disp(size(Zeta));  % 720 330
% disp(size(Temp));  % 720 330 50
% disp(size(Salt));  % 720 330 50
% disp(size(Uvel));  % 720 330 50
% disp(size(Vvel));  % 720 330 50
...
% MyTime=time/86400-(datenum('1-Jan-2007')-datenum('11-Oct-2006'))-0.5;
  MyTime=time -(datenum('1-Jan-1990')-datenum('1-Jan-1980'));  % days since 1-Jan-1990.

  if (BryRec == 0),
  % S.bry_time = 0;  % to bound initilization on 01-Jan-2007 00:00:00
    S.bry_time = MyTime*86400; % units: seconds since 1990-01-01 00:00:00
  else,
    S.bry_time = MyTime*86400;
  end,

  % overwrite the value above to make it the same as SODA3, i.e. 
  % days since 1980-01-01 00:00:00, so no conversion is needed.
  S.bry_time = time


% added by Ligang: change Tlon etc. from 0~360 to -180 ~ 180
% change Tlon, Tlat etc. from 1D to 2D to satisfy obc_mercator interface.
  Tlon_new = mod(Tlon + 180, 360) - 180;
  [Tlon_new, idx] = sort(Tlon_new);
  Temp_new = Temp(idx, :, :);
  Salt_new = Salt(idx, :, :);
  Zeta_new = Zeta(idx, :);
% disp('Tlon_new(1:5): ');
% disp(Tlon_new(1:5));
% disp(Tlon_new(end-4:end));

  [Tlon_new_2d, Tlat_new_2d] = ndgrid(Tlon_new, Tlat);
% disp(Tlon_new_2d(1:5, 1:2));
% disp(Tlat_new_2d(1:5, 1:2));

  Ulon_new = mod(Ulon + 180, 360) - 180;
  [Ulon_new, idx] = sort(Ulon_new);
  Uvel_new = Uvel(idx, :, :);
  Vvel_new = Vvel(idx, :, :);
% disp('Ulon_new(1:5): ');
% disp(Ulon_new(1:5));
% disp(Ulon_new(end-4:end));  

  [Ulon_new_2d, Ulat_new_2d] = ndgrid(Ulon_new, Ulat);


%  Determine Mercator Land/Sea mask.  Since Mercator is a Z-level
%  model, the mask is 3D.  
  Rmask3d=ones(size(Temp_new));
% size(Rmask3d);
% ind=find(Temp == 0);
  missing_value = -1e20;
  ind=find(Temp_new == missing_value);
  Rmask3d(ind)=0;
 %disp('Rmask3d: ')
 %disp(Rmask3d);
...
% disp(size(Zeta));  % 720 330
% disp(size(Tlon));  % 720 1
% disp(Tlon);  % 0.25 0.75 ... 359.75
% disp(size(Tlat));  % 330 1 
% disp(Tlat);  % -74.75 -74.25 ... 89.75
% disp(size(Rmask3d));  % 720 330 50

  zeta=obc_mercator('zeta',S,Zeta_new,Tlon_new_2d,Tlat_new_2d,Rmask3d(:,:,1));
...
% Vrho=obc_mercator('v'   ,S,Vvel,Vlon,Vlat,Rmask3d,Vdepth);
  ...
  temp=obc_mercator('temp',S,Temp_new,Tlon_new_2d,Tlat_new_2d,Rmask3d,Tdepth);
  salt=obc_mercator('salt',S,Salt_new,Tlon_new_2d,Tlat_new_2d,Rmask3d,Tdepth);
  Urho=obc_mercator('u'   ,S,Uvel_new,Ulon_new_2d,Ulat_new_2d,Rmask3d,Udepth);
  Vrho=obc_mercator('v'   ,S,Vvel_new,Ulon_new_2d,Ulat_new_2d,Rmask3d,Vdepth);
