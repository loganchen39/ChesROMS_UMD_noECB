      MODULE biology_mod
!
!git $Id: d06fee85 2025-06-28 03:05:36 UTC arango $
!=======================================================================
...
# endif
     ...
     &               FORCES(ng) % bvstr,                                &
     &               OCEAN(ng) % pH,                                    &
#ifdef GAS_TRANSFER_VELOCITY_HO2016
     &               OCEAN(ng) % u,                                     &
     &               OCEAN(ng) % v,                                     &
#endif
#ifdef DIAGNOSTICS_BIO
...
# endif
# ifdef MASK_WETLANDS_SAV
      real(r8), intent(in) :: mkws(LBi:,LBj:)
# endif
...
# endif
# ifdef MASK_WETLANDS_SAV
      real(r8), intent(in) :: mkws(LBi:UBi,LBj:UBj)
# endif
...
!
      ...
      real(r8) :: Att, AttFac, ExpAtt, Itop, PAR, chra
...
!
                TSS=TSS*2.9_r8               ! psl20250508; assume no organic in FSS.
...
#ifdef CHLA_AS_A_DIAGNOSTIC
!  Enforce a spatially-variable carbon-to-chlorophyll-a ratio based on Fig.3 of
!    Cerco&Noel 2004 (https://www.jstor.org/stable/24867907) and treat chl-a
!    as a fully diagnostic variable.
!
                chra=Att/(z_w(i,j,k)-z_w(i,j,k-1)) ! Diff.light att.coeff (m-1)
                chra=1._r8/(30._r8+150._r8*exp(-1.18_r8*chra)) !mg-chla (mg-C)-1
                cff=PhyCN(ng)*12.0_r8
                Bio(i,k,iChlo)=Bio(i,k,iPhyt)*cff*chra         !mg-chla m-3.
#else
!
                cff=PhyCN(ng)*12.0_r8
...
!  Lomas et al., 2002.
!
                Vp=max(Vp0(ng)*exp(0.08065_r8 * Bio(i,k,itemp)),3.00_r8) !psl20250715.
...
#ifdef GAS_TRANSFER_VELOCITY_HO2016
!           N.B. We are skipping the average at the cell-center.
!             See Eq.7 in https://doi.org/10.1002/2016GL068727
            cff3=sqrt( u(i,j,N(ng),nstp)**2 + v(i,j,N(ng),nstp)**2 )     ! m/s.
            cff3=0.266_r8 * u10squ + 0.77_r8 * sqrt( cff3 * 100._r8 )   &
     &          / sqrt( z_w( i, j, N(ng) ) - z_w( i, j, 0 ) )            ! cm/hr.
            cff3=cff3*dtdays*24.0_r8/100.0_r8*SQRT(660.0_r8/SchmidtN_Ox) ! m.
#else
...
# if defined ORGANIC_ALK_CHALK_YORK || defined ORGANIC_ALK_CHALK_POTOMAC || defined ORGANIC_ALK_CHALK_BAYWIDE
                              ng,                                       &
# endif
# ifdef MASKING
     &                        rmask,                                    &
...
!             See Eq.7 in https://doi.org/10.1002/2016GL068727
            ...
            cff3=0.266_r8 * u10squ + 0.77_r8 * sqrt( cff3 * 100._r8 )   &
     &          / sqrt( z_w( i, j, N(ng) ) - z_w( i, j, 0 ) )            ! cm/hr.
            cff3=cff3 * dtdays*24.0_r8/100.0_r8 *SQRT(660.0_r8/SchmidtN) ! m.
#else
            cff3=cff2*u10squ*SQRT(660.0_r8/SchmidtN)
#endif
#ifdef CHEM_ENHANCEMENT_CO2_AIR_SEA_FLUX
!           Enforce a minimum gas transfer velocity of 4cm/hr (K.Hinson pers.comm. 2024-09-25)
!             to represent chemical enhancement (e.g. Wanninkhof & Knox 1996).
!           cff3 is equivalent to Eq.4 of Wanninkhof 2014 (originally in cm/hr) scaled by
!             x 24hrs/day / 100cm/m x dtdays.
            cff3 = max( cff3, 4._r8 * 24._r8 / 100._r8 * dtdays )
#endif
!
...
!
     ...
     &               cff1*Hz(i,j,1)*(365.0_r8/dtdays) ! gC m-2 year-1
                Cbe=MIN(0.00_r8,0.023_r8*fac1**0.5797_r8 ) !psl20250702
...
# if defined ORGANIC_ALK_CHALK_YORK || defined ORGANIC_ALK_CHALK_POTOMAC || defined ORGANIC_ALK_CHALK_BAYWIDE
!     ng         Grid index (if multiple grids)                        !
# endif
...
# if   defined ORGANIC_ALK_CHALK_YORK
!       Pers.comm. 2025-01-28 Sophie Kuhl (skhul@whoi.edu)
        if     (                      yday <=  91._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.12_r8 - 0.00649_r8 * S(i))) ! WI2024
        elseif ( yday >  91._dp .and. yday <= 183._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.11_r8 - 0.00676_r8 * S(i))) ! SP2024
        elseif ( yday > 183._dp .and. yday <= 274._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.13_r8 - 0.00623_r8 * S(i))) ! SU2023
        else
          alk = alk * (1._r8 - max(0._r8, 0.09_r8 - 0.00403_r8 * S(i))) ! FA2023
        end if
# elif defined ORGANIC_ALK_CHALK_POTOMAC
!       Pers.comm. 2025-01-28 Sophie Kuhl (skhul@whoi.edu)
        if     (                      yday <=  91._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.03_r8 - 0.00081_r8 * S(i))) ! WI2024
        elseif ( yday >  91._dp .and. yday <= 183._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.02_r8 - 0.00061_r8 * S(i))) ! SP2024
        elseif ( yday > 183._dp .and. yday <= 274._dp ) then
          alk = alk * (1._r8 - max(0._r8, 0.02_r8 - 0.00072_r8 * S(i))) ! SU2023
        else
          alk = alk * (1._r8 - max(0._r8, 0.02_r8 - 0.00056_r8 * S(i))) ! FA2023
        end if
# elif defined ORGANIC_ALK_CHALK_BAYWIDE
!       Pers.comm. 2025-01-28 Sophie Kuhl (skhul@whoi.edu)
        if     (                      yday <=  91._dp ) then
          alk = alk - max(34.75_r8 - 0.2765_r8 * S(i), 0._r8) * 1.e-6_r8! WI2024
        elseif ( yday >  91._dp .and. yday <= 183._dp ) then
          alk = alk - max(27.50_r8 - 0.3968_r8 * S(i), 0._r8) * 1.e-6_r8! SP2024
        elseif ( yday > 183._dp .and. yday <= 274._dp ) then
          alk = alk - max(35.45_r8 - 1.4000_r8 * S(i), 0._r8) * 1.e-6_r8! SU2023
        else
          alk = alk - max(36.30_r8 - 0.5895_r8 * S(i), 0._r8) * 1.e-6_r8! FA2023
        end if
# endif
!
!-----------------------------------------------------------------------
