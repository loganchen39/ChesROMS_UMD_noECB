...
!
      ...
      real(r8), parameter :: SPONGE_WIDTH = 15.0_r8   ! grid points
!
...
#else
!
!  Chesbay600m boundary-sponge nudging coefficients.  The default ROMS
!  template fills the coefficient array UNIFORMLY across the whole
!  domain.  That is dangerous here because LnudgeTCLM/LnudgeM3CLM are
!  used for a SECOND purpose in step3d_t.F/rhs3d.F: a full 3D interior
!  relaxation of tracers/momentum toward CLIMA(ng)%tclm/uclm/vclm
!  (currently placeholder zeros, see ana_tclima.h/ana_m3clima.h), on
!  top of the intended open-boundary nudging in t3dbc_im.F/u3dbc_im.F.
!  A uniform coefficient would therefore nudge the ENTIRE domain's
!  temperature/salinity/momentum toward zero every timestep. Instead,
!  ramp the coefficient from its full .in-specified value right at the
!  open boundaries (south, east) down to zero within SPONGE_WIDTH grid
!  points, so the unwanted interior term is negligible away from the
!  boundary while the intended OBC nudging (which only reads the
!  coefficient at the boundary row itself) is unaffected.
!
      DO j=JstrT,JendT
        DO i=IstrT,IendT
          wrk(i,j)=MAX(0.0_r8,                                          &
     &                 (SPONGE_WIDTH-REAL(j,r8))/SPONGE_WIDTH)
          wrk(i,j)=MAX(wrk(i,j),                                        &
     &                 MAX(0.0_r8,                                      &
     &                     (SPONGE_WIDTH-                               &
     &                      REAL(Lm(ng)+1-i,r8))/SPONGE_WIDTH))
        END DO
      END DO
      ...
      IF (LnudgeM2CLM(ng)) THEN
        DO j=JstrT,JendT
          DO i=IstrT,IendT
            CLIMA(ng)%M2nudgcof(i,j)=wrk(i,j)*M2nudg(ng)
          END DO
        END DO
      END IF
...
!
      IF (LnudgeM3CLM(ng)) THEN
        DO k=1,N(ng)
          DO j=JstrT,JendT
            DO i=IstrT,IendT
              CLIMA(ng)%M3nudgcof(i,j,k)=wrk(i,j)*M3nudg(ng)
            END DO
          END DO
        END DO
      END IF
      ...
      IF (ANY(LnudgeTCLM(:,ng))) THEN
        DO itrc=1,NTCLM(ng)
          DO k=1,N(ng)
            DO j=JstrT,JendT
              DO i=IstrT,IendT
                CLIMA(ng)%Tnudgcof(i,j,k,itrc)=wrk(i,j)*Tnudg(itrc,ng)
              END DO
            END DO
          END DO
        END DO
