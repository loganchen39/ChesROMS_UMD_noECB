...
!=======================================================================
!                                                                      !
!  This routine reads in the Estuarine Carbon Biogeochemistry model    !
!  (Feng et al., 2015, and follow-ups) input configuration parameters. !
!  They are specified in input script "bio_ECB.in".                    !
!                                                                      !
!=======================================================================
!
      USE mod_param
...
!
!-----------------------------------------------------------------------
!  Read in the Estuarine Carbon Biogeochemistry model (Feng et al.,
      ...
      DO WHILE (.TRUE.)
        ...
        IF (status.gt.0) THEN
          SELECT CASE (TRIM(KeyWord))
            ...
            CASE ('deltC')
              Npts=load_r(Nval, Rval, Ngrids, deltC)
            CASE ('gammaC')
              Npts=load_r(Nval, Rval, Ngrids, gammaC)
            CASE ('slCexc')
              Npts=load_r(Nval, Rval, Ngrids, slCexc)
            CASE ('pCO2air')
...
# endif
            ...
            CASE ('Dout(iNO3u)')
              ...
              END DO
           CASE ('Dout(iPPCe)')
             IF (iDbio3(iPPCe).eq.0) THEN
               IF (Master) WRITE (out,40) 'iDbio3(iPPCe)'
                 exit_flag=5
               RETURN
             END IF
             Npts=load_l(Nval, Cval, Ngrids, Lbio)
             ...
             END DO
            CASE ('Dout(iwdeN)')
              IF (iDbio3(iwdeN).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio3(iwdeN)'
                  exit_flag=5
                  RETURN
                END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
              ...
              END DO
            CASE ('Dout(iSoxy)')
              IF (iDbio2(iSoxy).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iSoxy)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
!
     ...
     &            'Temperature-limited phytoplankton growth parameter', &
     &            '(day-1).'
     ...
     &            'Zooplankton maximum growth rate at 0C (day-1).'
     ...
     &            'Coagulation rate (m3 mmol_N-1 day-1).'
     ...
     &            'Remineralization rate for semilabileDON (day-1).'
            WRITE (out,80) RDeRRC(ng), 'RDeRRC',                        &
     &            'Remineralization rate for semilabileDOC (day-1).'
     ...
     &            'light attenuation by CDOM in freshwater (S=0) (m-1).' 
            WRITE (out,80) rkdTSS1(ng), 'rkdTSS1',                      &
     &            'light attenuation by TSS (m2 g-1).'
            WRITE (out,80) rkdS1(ng), 'rkdS1',                          &
     &            'light attenuation by CDOM (slope) (m-1 PSU-1).'
     ...
     &            'phyto cell (nondimensional).'
            WRITE (out,90) deltC(ng), 'deltC',                          &
     &            'Fraction of semilabile DOC to total DOC in the ',    &
     &            'phyto cell (nondimensional).'
            WRITE (out,90) gammaC(ng), 'gammaC',                        &
     &            'Parameter of carbon excess-based DOC exudation ',    &
     &            '(nondimensional).'
            WRITE (out,90) slCexc(ng), 'slCexc',                        &
     &            'Fraction of semilabile DOC produced by carbon ',     &
     &            'excess uptake/exudation (nondimensional).'
...
# endif
     ...
     &              'Write out tracer ', i, TRIM(Vname(1,idTvar(i)))
              ...
              DO itrc=1,NBT
                i=idbio(itrc)
                IF (Hout(idzslT(i),ng)) WRITE (out,120)                 &
     ...
     &              'Write out tracer ', i, TRIM(Vname(1,idTvar(i)))
              ...
              DO itrc=1,NBT
                i=idbio(itrc)
                IF (Qout(idzslT(i),ng)) WRITE (out,120)                 &
     ...
     &              TRIM(Vname(1,idTvar(i)))
              ...
              DO itrc=1,NBT
                i=idbio(itrc)
                IF (Qout(idqboT(i),ng)) WRITE (out,120)                 &
     &              Qout(idqboT(i),ng), 'Qout(idqboT)',                 &
     &              'Write out bottom  tracer ', i,                     &
     &              TRIM(Vname(1,idTvar(i)))
              END DO
#endif
              DO itrc=1,NBT
...
!
  ...
  60  FORMAT (/,/,' ECB Model Parameters, Grid: ',i2.2,                 &
     &        /,  ' ==============================',/)
