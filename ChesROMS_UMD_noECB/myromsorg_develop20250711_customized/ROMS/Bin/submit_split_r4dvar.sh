...
################################################## Hernan G. Arango ###
##                                                                    #
## ROMS Split R4D-Var Data Assimilation running BASH script:          #
##                                                                    #
## The R4D-Var is split in several phases and executables:            #
##                                                                    #
##   1) background                          ROMS_EXE_A                #
##   2) increment                           ROMS_EXE_B                #
##   3) analysis                            ROMS_EXE_B                #
...
##             RPM, TLM, and ADM kernels used in the 4D-Var           #
##             minimization. It interpolates the NLM solution to the  #
##             observation locations in space and time. The NLM could #
##             be a part of a coupling system and or include nested   #
##             grids. Notice that "analysis" phase the representer    #
##             model is run instead of the nonlinear model. Therefore,#
##             the RPM trajectory is used when outer>1.               #
##                                                                    #
## ROMS_EXE_B: It is used in the "increment", "analysis", and         #
##             "posterior_error" phases. The 4D-Var increment is      #
##             obtained by minimizing the cost function over Ninner   #
##             loops. It is possible to use a coarser grid resolution #
##             in the iterations of the inner loops. If so, the       #
##             "background" trajectory needs to be interpolated into  #
##             the coarser grid. Then, after the inner loops, the     #
##             coarser grid increment needs to be interpolated to the #
##             finer grid before the "analysis" phase. The "increment"#
##             phase could be run at a lower precision.               #
##                                                                    #
## Notice that this script has a customizable function, My4DVarScript,#
## at the top that creates "r4dvar.in" from a template, which is      #
## overwritten in each phase. The "rdvar.in" passes to ROMS the phase #
## and outer loop value to compute.                                   #
##                                                                    #
## Then, there is the customizable section for the computer batch     #
## directives and the tunable parameters.                             #
##                                                                    #
## R4D-Var phases workflow:                                           #
##                                                                    #
##   CALL prior_error                                                 #
##                                                                    #
...
##
##  Use 'sbatch submit_split_r4dvar.sh'    to queue the job
...
#SBATCH --job-name=ROMS_split_r4dvar     # Assign an short name to your job
...
#######################################################################
## R4D-Var data assimilation input script function.  It generates     #
## 'r4dvar.in' from the 's4dvar.in' template.                         #
#######################################################################
...
## Create ROMS 4D-Var input script 'r4dvar.in' from template.
