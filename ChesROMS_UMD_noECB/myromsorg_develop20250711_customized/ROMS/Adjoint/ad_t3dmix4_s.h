...
!
#ifdef PROFILE
      CALL wclock_on (ng, iADM, 27, __LINE__, MyFile)
#endif
      CALL ad_t3dmix4_s_tile (ng, tile,                                 &
     &                        LBi, UBi, LBj, UBj,                       &
...
!!   &                        DIAGS(ng) % DiaTwrk,                      &
     ...
     &                        OCEAN(ng) % ad_t)
#ifdef PROFILE
      CALL wclock_off (ng, iADM, 27, __LINE__, MyFile)
#endif
!
      RETURN
...
!
      RETURN
      END SUBROUTINE ad_t3dmix4_s_tile
