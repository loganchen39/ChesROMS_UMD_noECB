...
!
# ifdef MB_BBL_IMPOSE_Z0
           ZnotC=ZoBot(i,j)!psl20250807: Use ZoBot(i,j) in all circumstances.
# else
           ZnotC=d50/12.0_r8
# endif
...
# if defined MB_Z0BL && !defined(MB_BBL_IMPOSE_Z0)
!
!-----------------------------------------------------------------------
...
!
#  if defined MB_CALC_ZNOT && !defined(MB_BBL_IMPOSE_Z0)
...
#  endif
# endif
# if defined MB_BBL_IMPOSE_MIN_Z0 && !defined(MB_BBL_IMPOSE_Z0)
            ZnotC=MAX(ZoBot(i,j),ZnotC) !psl20250625
# endif
            cff1=vonKar/LOG(Zr(i,j)/ZnotC)
            cff2=MIN(Cdb_max,MAX(Cdb_min,cff1*cff1))
!!          tauc(i,j)=cff2*Umag(i,j)*Umag(i,j)
