...
!
     ...
     &                          pioFile = pioFile)
      ...
      CALL pio_netcdf_put_fvar (ng, model, ncname, 'K_ext',             &
     &                          K_ext(ng), (/0/), (/0/),                &
     &                          pioFile = pioFile)
      IF (FoundError(exit_flag, NoError, __LINE__, MyFile)) RETURN
     ...
     &                          pioFile = pioFile)
      ...
      CALL pio_netcdf_put_fvar (ng, model, ncname, 'ZooMD',             &
     &                          ZooMD(ng), (/0/), (/0/),                &
     &                          pioFile = pioFile)
      IF (FoundError(exit_flag, NoError, __LINE__, MyFile)) RETURN

      CALL pio_netcdf_put_fvar (ng, model, ncname, 'ZooGA',             &
     &                          ZooGA(ng), (/0/), (/0/),                &
     &                          pioFile = pioFile)
      IF (FoundError(exit_flag, NoError, __LINE__, MyFile)) RETURN

      CALL pio_netcdf_put_fvar (ng, model, ncname, 'ZooEC',             &
     &                          ZooEC(ng), (/0/), (/0/),                &
     &                          pioFile = pioFile)
      IF (FoundError(exit_flag, NoError, __LINE__, MyFile)) RETURN
